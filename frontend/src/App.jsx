// src/App.jsx - UPDATED SINGLE FILE VERSION
...
const ProtectedRoute = ({ children, requireAdmin = false }) => {
    ...
    if (!user) return null;
    if (requireAdmin && !user.role?.includes('admin')) return null;
...
const GuestRoute = ({ children }) => {
    ...
    useEffect(() => {
        if (!loading && user) {
            navigate(user.role?.includes('admin') ? "/admin/dashboard" : "/dashboard", { replace: true });
        }
...
const toasterOptions = {
    ...
    success: {
        duration: 3000,
        iconTheme: { primary: ENV.SECONDARY_COLOR, secondary: '#fff' },
        style: {
...
function AppContent() {
    ...
    useEffect(() => {
        ...
        console.log(`Mode: ${ENV.APP_MODE}, API: ${ENV.API_URL}, AI: ${ENV.ENABLE_AI}`);
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-900 to-blue-900 text-white flex items-center justify-center p-4">
                <div className="max-w-lg text-center">
                    <h1 className="text-4xl font-bold mb-4">⚙️ Under Maintenance</h1>
                    <p className="text-xl text-blue-200 mb-6">
                        {ENV.APP_NAME} is currently undergoing scheduled maintenance.
                    </p>
                    <p className="text-sm text-blue-300">v{ENV.APP_VERSION}</p>
                </div>
            </div>
        );
    }
            ...
            <Suspense fallback={<PageLoading />}>
                <Routes>
                    ...
                    <Route element={<AdminLayout />}>
                        ...
                        <Route path="/admin/resumes" element={<ProtectedRoute requireAdmin><AdminResumes /></ProtectedRoute>} />
        ...
        <QueryClientProvider client={queryClient}>
            ...
            {import.meta.env.DEV && (
                <ReactQueryDevtools
                    ...
                    toggleButtonProps={{ style: { margin: '10px', opacity: 0.9 } }}
                />
            )}
        </QueryClientProvider>
    );
