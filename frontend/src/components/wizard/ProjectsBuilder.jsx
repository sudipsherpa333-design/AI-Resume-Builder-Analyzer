...
const ProjectsBuilder = ({
    ...
    const calculateImpactScore = (description) => {
        ...
        // Check for metrics
        if (/\d+%/.test(desc)) score += 20;
        if (/\$\d+/.test(desc)) score += 20;
        if (/\d+\s*(users|clients|customers)/i.test(desc)) score += 20;
        if (/increased|decreased|improved|reduced/i.test(desc)) score += 20;
        if (/scalable|efficient|optimized/i.test(desc)) score += 20;
