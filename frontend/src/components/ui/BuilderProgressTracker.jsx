...
const ProgressIndicator = ({ resumeData = {} }) => {
    ...
    const progressData = useMemo(() => {
        ...
        const suggestions = [];
        if (!resumeData?.personalInfo?.fullName) suggestions.push('Add your name');
        if (!resumeData?.personalInfo?.email) suggestions.push('Add your email');
        if (!resumeData?.summary?.trim()) suggestions.push('Write a summary');
        if ((resumeData?.experience?.length || 0) === 0) suggestions.push('Add work experience');
        if ((resumeData?.education?.length || 0) === 0) suggestions.push('Add education');
        if ((resumeData?.skills?.length || 0) === 0) suggestions.push('Add skills');
