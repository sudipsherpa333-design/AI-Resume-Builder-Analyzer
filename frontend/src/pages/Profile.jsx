        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border border-gray-100 lg:sticky lg:top-8">
              ...
              <div className="text-center mb-6">
                <div className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-4 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white text-4xl font-bold overflow-hidden relative border-4 border-white shadow-md">
                  {(profileData.profilePicture) ? (
                    <img src={profileData.profilePicture} alt="Profile" className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; }} />
                  ) : (
                ...
                <div className="mt-4">
                  <button
                    type="button"
                    onClick={() => { setActiveTab('profile'); setIsEditing(true); fileInputRef.current && fileInputRef.current.click(); }}
              ...
              {/* Logout Button */}
              <button onClick={() => { logout(); navigate('/'); }} className="w-full px-4 py-3 bg-red-50 text-red-600 font-semibold rounded-lg hover:bg-red-100 transition-all duration-200 shadow-md flex items-center justify-center">
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-6">
                  ...
                  {isEditing && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row gap-3 pt-4 w-full">
                      ...
                      <button type="button" onClick={() => { setIsEditing(false); setProfileData({ name: user.name, email: user.email, profilePicture: user.profilePicture || '', bio: user.bio || '' }); setAvatarFile(null); }} className="px-6 py-3 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-colors font-semibold w-full sm:w-auto justify-center">Cancel</button>
                    </motion.div>
                  )}
                </form>
              </div>
