...
export const useAIAssistant = (options = {}) => {
  ...
  const getSuggestions = useCallback(async (sectionId, currentData) => {
    ...
    // Defensive: detect if first arg is object (resume) instead of string (section)
    if (typeof sectionId === 'object' && sectionId !== null) {
      console.warn('⚠️ [useAIAssistant] getSuggestions called with wrong arg order (resume object first), skipping');
      return [];
    }

    // Ensure sectionId is a string
    if (typeof sectionId !== 'string') {
      console.warn('⚠️ [useAIAssistant] getSuggestions called with invalid sectionId type:', typeof sectionId);
      return [];
    }
